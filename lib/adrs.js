...
const adr_re = new RegExp(env.GITHUB_ADR_REGEX);
const fullPathAdr_re = new RegExp(env.GITHUB_PATH_TO_ADRS + "/" + env.GITHUB_ADR_REGEX);
// gets the contents of the adr directory
const adrContents = `
  {
    repository(name: "${env.GITHUB_REPO}", owner: "${env.GITHUB_USER}") {
      object(expression: "${env.GITHUB_DEFAULT_BRANCH}:${env.GITHUB_PATH_TO_ADRS}") {
        ... on Tree {
          entries {
            name
            object {
...
export async function getAdrFiles() {
  ...
  await adrFilesRaw.reduce(async (prev,adrFileRaw) => {
    ...
    if (adr_re.test(fileName))
    {
      ...
      + "/blob/" + env.GITHUB_DEFAULT_BRANCH
      + "/" + env.GITHUB_PATH_TO_ADRS + "/" + fileName;
