 ...
 * creates a new branch off the given ref
 */
 ...
 * gets the contents of the new-adr-template file, rewrites the title and header
...
async function getTemplateContents() {
  ...
  } = await octokit.graphql(decisionTemplateQuery,{
    owner: process.env.GITHUB_USER,
    repo: process.env.GITHUB_REPO,
...
const commitNewADRQuery = `mutation (
    $branchName: String!, 
...
async function commitNewADR(branch,headline,expectedOid,path,text) {
  ...
  const oid = await octokit.graphql(commitNewADRQuery,
    {
      branchName: branch,
 ...
 * uses the title to create a short name for the file
 */

async function getNewADRFilename(title) {
  ...
  return str_id + "-" + title.replace(/\s/g,"-",).substring(0,50) + ".md";
}
...
export async function createAdrFile(options) {
  ...
  const {
    title = "My new ADR title",
    branch = "my-new-branch-name",
    impact = "high"
  } = options;

  const mainSHA = await getMainSHA();

  //const postResult = await createNewBranch(branch,mainSHA);
  ...
  // use the title as the headline
  const headline = title;

  const path = process.env.GITHUB_PATH_TO_ADRS + "/" + await getNewADRFilename(title);

  //commitNewADR(branch,headline,mainSHA,path,newAdrContents);
